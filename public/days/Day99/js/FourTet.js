...
function init() {
    ...
    gui.add(options, "maxConn", 5,  500).listen().onChange((value)=>{
            maxConnectionNodes = value;
        }
    );
...
function addIndexToMove(){
    currentIndexToMove += 10;
    if (currentIndexToMove < 2000){
        maxConnectionNodes = 5 + 495 * (currentIndexToMove/matrix.length);
    }
    else{
        minConnectionNodes = 0;
        maxConnectionNodes = 5;
    }
}
...
function animate() {
    for (let i=0; i<matrix.length; i++){
        matrix[i].update();
        if (i< currentIndexToMove || i>matrix.length - currentIndexToMove){
            matrix[i].startRotate = true;
        }
    }
