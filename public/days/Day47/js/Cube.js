class Cube {
    constructor(start_x, start_y, start_z, halfLengthFront, halfLengthSide, halfLengthDepth, rotationAxis, positiveRotation){
        ...
        this.startX = start_x;
        this.startZ = start_z;
        this.horizontalVector = new THREE.Vector3(this.startX, 0, this.startZ);
        this.horizontalVector.normalize();
        this.scale = 10;
        this.horizontalVector.x = this.scale*Math.random()*this.horizontalVector.x;
        this.horizontalVector.y = this.scale*Math.random()*this.horizontalVector.y;
        this.horizontalVector.z = this.scale*Math.random()*this.horizontalVector.z;
        this.x = start_x;
        this.y = start_y;
        this.z = start_z;
        ...
        this.upThreshold = this.startY + 800;
    ...
    update(){
        if (this.y < this.upThreshold){
            this.y += this.upSpeed;
            this.x += this.horizontalVector.x;
            this.z += this.horizontalVector.z;
            this.cubeCenter.position.copy(new THREE.Vector3(this.x, this.y, this.z));
        }
        else{
