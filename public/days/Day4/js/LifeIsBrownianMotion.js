...
let lifePoints = [];
//Camera
let sidePosition = new THREE.Vector3(4500, 300, 9);
...
function init() {
    ...
    createLifePoints();
    setTimeout(moveToTheSide, 3000);
}


function moveToTheSide(){
    moveCamera(sidePosition, 15000);
}
...
function createLifePoints(){
    ...
    for (let i=0; i<rowLives; i++){
        ...
        for (let j=0; j<colLives; j++){
            ...
            scene.add(newLifePoint.lineProcess);
...
function animate() {
    ...
    for (let i=0; i<lifePoints.length; i++){
        if (Math.random() > 0.8){
            lifePoints[i].on = !lifePoints[i].on;
        }
        lifePoints[i].update();
    }
    TWEEN.update();
