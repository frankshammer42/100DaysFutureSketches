...
function init() {
    ...
    camera.position.x = -4183.69;
    camera.position.y = -6645;
    camera.position.z = 23057;
    camera.far = 1000000;
...
function animate() {
    ...
    let length = curves[0].curveCircles.length;
    for (let i=0; i<curves.length; i++){
        if (drawProcess < curves[i].curveCircles.length){
            scene.add(curves[i].curveCircles[length-drawProcess]);
        }
        // curves[i].update();
    }
    drawProcess+=1;
