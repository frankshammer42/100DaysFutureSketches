...
class Curve{
    ...
    scaleCurveCircles(scale){
        let circleSegment = 64;
        let radius = Math.abs(Math.sin(this.initRadius*scale))*1000;
        this.initRadius = radius;
        for (let i=0; i<this.curveCircles.length; i++){
            let curveCircle = this.curveCircles[i];
            let center = this.curveData[i];
            let current_positions = curveCircle.geometry.attributes.position.array;
            for (let i=0; i<=circleSegment; i++){
                let theta = (i / circleSegment) * Math.PI * 2;
                let x = center.x;
                let y = Math.sin(theta) * radius + center.y;
                let z = Math.cos(theta) * radius + center.z;
                current_positions[i*3] = x;
                current_positions[i*3+1] = y;
                current_positions[i*3+2] = z;
            }
            curveCircle.geometry.attributes.position.needsUpdate = true;
        }
    }
    ...
    update(){
        // this.scaleCurveCircles(1.00002);
